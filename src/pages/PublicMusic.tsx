...
type MusicRequest = {
  ...
  artist: string | null;
  note: string | null;
  status: "Neu" | "Geplant" | "Gespielt" | "Abgelehnt";
  likes: number;
  created_at: string;
};
...
export default function PublicMusic() {
  ...
  async function submitRequest(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!cleanGuestName || !cleanSongTitle) {
      setErrorMessage("Bitte gib deinen Namen und einen Songtitel ein.");
      return;
    }
    ...
    setArtist("");
    setNote("");
    setSuccessMessage("Dein Musikwunsch wurde eingereicht.");
    setSubmitting(false);

    await loadRequests();
  }

  async function likeRequest(request: MusicRequest) {
    const storageKey = `partycontrol-like-${request.id}`;

    if (localStorage.getItem(storageKey)) {
      setErrorMessage("Du hast diesen Song bereits geliked.");
      return;
    }
      ...
      <main className="mx-auto max-w-6xl p-4 sm:p-6 lg:p-10">
        <section className="mb-7">
          ...
          <p className="mt-3 max-w-2xl text-zinc-400">
            Reiche deinen Musikwunsch ein oder unterstütze bereits gewünschte
            Songs mit einem Like.
          </p>
        </section>

        <div className="grid grid-cols-1 gap-6 xl:grid-cols-[0.9fr_1.1fr]">
          <section className="h-fit rounded-3xl border border-zinc-800 bg-zinc-950 p-5 sm:p-7">
            <div className="flex items-center gap-3">
              <Music2 className="text-yellow-400" size={24} />
              ...
              <div>
                ...
                <p className="text-sm text-zinc-500">
                  Name und Songtitel sind Pflichtfelder.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-500">
                  Die beliebtesten Wünsche stehen oben.
                </p>
              </div>

              <div className="rounded-full bg-yellow-400/10 px-4 py-2 font-bold text-yellow-400">
            ...
            <div className="mt-6 space-y-3">
              ...
              {!loading &&
                sortedRequests.map((request, index) => (
                  ...
                  >
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-yellow-400/10 font-black text-yellow-400">
                      {index + 1}
                    </div>
                    ...
                    <button
                      type="button"
                      onClick={() => likeRequest(request)}
                      className="flex min-h-11 shrink-0 items-center gap-2 rounded-xl border border-zinc-700 bg-zinc-950 px-3 py-2 font-bold text-zinc-300 transition hover:border-yellow-400 hover:text-yellow-400"
                    >
                      <Heart size={18} />
                      {request.likes}
                    </button>
